-- Idempotent: adds `rotations.cycle_week` and refreshes `upsert_rotation` if an older DB
-- never received `20260412090000_yotei_cycle_week.sql` (safe to apply after that migration too).

alter table rotations
  add column if not exists cycle_week smallint
  check (cycle_week is null or (cycle_week >= 1 and cycle_week <= 12));

comment on column rotations.cycle_week is 'Ghost of Yōtei survival rotation cycle week (1–12); optional for legacy data.';
...
declare
  ...
  v_cycle_week smallint := (nullif(trim(coalesce(payload->>'cycle_week', '')), ''))::smallint;
  v_existing_id uuid;
  ...
  v_challenge jsonb;
begin
  if v_cycle_week is not null and (v_cycle_week < 1 or v_cycle_week > 12) then
    raise exception 'cycle_week must be between 1 and 12 or null';
  end if;
  ...
  if v_rotation_id is not null then
    ...
    if v_existing_id is not null then
      ...
      insert into rotations (map_id, week_start, credit_text, cycle_week)
        values (v_map_id, v_week_start, v_credit_text, v_cycle_week)
