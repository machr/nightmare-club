...
type RotationApiContext = {
	supabase: SupabaseClient;
};

export async function buildRotationApiResponse({ supabase }: RotationApiContext) {
	...
	for (const map of maps) {
		...
		const rounds =
			rotation?.rounds
				?.sort((a: { round_number: number }, b: { round_number: number }) => a.round_number - b.round_number)
				.map((round: { round_number: number; waves: any[] }) => {
					const roundChallenges = challengesByRound.get(round.round_number) ?? [];
					return {
						round: round.round_number,
						challenges: roundChallenges.map((c: any) => ({
							name: c.name,
							description: c.description
						})),
						waves: round.waves
							.sort((a: { wave_number: number }, b: { wave_number: number }) => a.wave_number - b.wave_number)
		...
		result.push({
			...
			updated_at: rotation?.created_at ?? null,
			rounds
		});
	}
