<script lang="ts">
	...
	function getExistingAttunement(roundNum: number, waveNum: number, spawnIdx: number, idx: number): string {
	...
	function hasSecondAttunement(roundNum: number, waveNum: number, spawnIdx: number): boolean {
		const key = spawnKey(roundNum, waveNum, spawnIdx);
		if (showSecondAttunement[key] !== undefined) return showSecondAttunement[key];
...
<div class="space-y-6">
	...
	<form method="POST" action="?/save" use:enhance class="space-y-8">
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
			<div class="space-y-2">
				<label for="map_id" class="text-sm font-semibold text-foreground">Map</label>
				<select
					id="map_id"
					name="map_id"
					class={selectClass}
			...
			<div class="space-y-2">
				<label for="challenge_id" class="text-sm font-semibold text-foreground">Challenge</label>
				<select
					id="challenge_id"
					class={selectClass}
			...
			<div class="space-y-2">
				<span class="text-sm font-semibold text-foreground">Week Start (Saturday)</span>
		...
		{#each Object.keys(ROUND_STRUCTURE).map(Number) as roundNum}
			...
			<div class="rounded-lg border-2 border-border p-4 space-y-4">
				...
				{#each Array.from({ length: waveCount }, (_, i) => i + 1) as waveNum}
					<div class="space-y-2">
						<h4 class="text-sm font-semibold text-foreground/70">Wave {waveNum}</h4>
						<div class="grid gap-3 {spawnCount === 3 ? 'grid-cols-3' : 'grid-cols-2'}">
							{#each Array.from({ length: spawnCount }, (_, i) => i + 1) as spawnIdx}
								...
								{@const key = spawnKey(roundNum, waveNum, spawnIdx)}
								<div class="rounded-md border-2 border-border/60 bg-card p-2 space-y-1.5">
									<div class="flex items-center gap-2">
										<span class="text-xs font-bold text-foreground/60 w-5">
											{spawnIdx}
										</span>
										<SegmentedControl
											options={locations}
										...
										<div class="flex items-center gap-2 pl-7">
											...
											{#if hasSecondAttunement(roundNum, waveNum, spawnIdx)}
												...
												<button
													...
													onclick={() => (showSecondAttunement[key] = false)}
												>
													&times;
												</button>
											{:else}
												<button
													...
													onclick={() => (showSecondAttunement[key] = true)}
												>
													+
												</button>
											{/if}
